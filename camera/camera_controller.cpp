    ...
    while (running) {
        ...
        if (cmd == "get") {
            if (camera_id > 0 && camera != NULL) {
                ...
                resp_first_camera_data["rotationx"] = camera->rotation_x/100;
                if ( 320<= camera->rotation_y/100<360) {
                    resp_first_camera_data["rotationy"] = camera->rotation_y/100 - 360;
                } else {
                    resp_first_camera_data["rotationy"] = camera->rotation_y/100;
                }
                ...
                for (auto& pair : cameras) {
                    ...
                    resp_one_camera_data["rotationx"] = camera->rotation_x/100;

                    if ( 320<= camera->rotation_y/100<360) {
                        resp_one_camera_data["rotationy"] = camera->rotation_y/100 - 360;
                    } else {
                        resp_one_camera_data["rotationy"] = camera->rotation_y/100;
                    }
...
int Camera::patrol_with_calibration_loop(bool is_calibrate) //return 0表示正常 非0表示异常
{
    ...
    for(auto position = preset_positions.begin(); position != preset_positions.end(); ++position) {
        auto px  = (position->rotation_x%360)%360;
        auto py = (360 - position->rotation_y) % 360;
...
int Camera::fetch_remote_status()
{
    ...
    std::thread th2([this,&res2](){
        ...
        if (rc == -1) {
            ...
            if (320<=regs[1]/100<360) {
                rotation_y = regs[1]/100 - 360;
            } else {
                rotation_y = regs[1]/100;
            }
            WTALOGI("摄像机[%d]同步当前姿态: x=%d, y=%d",id, rotation_x, rotation_y);
        }
