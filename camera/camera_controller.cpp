...
int Camera::patrol_with_calibration_loop(bool is_calibrate) //return 0表示正常 非0表示异常
{
    ...
    for(auto position = first_pos; position != preset_positions.end(); ++position, now_point_id++) {
        ...
        if (is_calibrate) { //标定模式
            ...
            // 重置拍照完成标志
            l0_captured.store(false);
            l1_captured.store(false);

            // 第一阶段：开灯拍照（L0 = 有灯照）
            set_brighten(position->brightness); // 开灯
            std::this_thread::sleep_for(std::chrono::milliseconds(500)); // 等待灯实际点亮稳定
            ...
            while (!l0_captured.load() && wait_ms < max_wait_ms) {
            ...
            // 第二阶段：关灯拍照（L1 = 无灯照）
            set_brighten(0); // 关灯
            std::this_thread::sleep_for(std::chrono::milliseconds(500)); // 等待灯实际熄灭稳定
            ...
            wait_ms = 0;
            while (!l1_captured.load() && wait_ms < max_wait_ms) {
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
                wait_ms += 50;
            }
            if (!l1_captured.load()) {
