...
int Camera::patrol_with_calibration_loop(bool is_calibrate) //return 0表示正常 非0表示异常
{
    ...
    for(auto position = first_pos; position != preset_positions.end(); ++position, now_point_id++) {
        ...
        while (!posture_completed && std::chrono::duration_cast<std::chrono::seconds>(
            ...
            update_posture_state(px, py); //更新姿态就位状态
            try_count++;
            if (posture_completed) {
                // noop
            } else {
                // 短暂休眠避免CPU过度占用，随尝试次数缩短间隔 (1秒 -> 0.5秒)
                int sleep_ms = std::max(500, 1000 - (try_count * 100));
        ...
        if (!posture_completed) {
            ...
            // L0 开灯标定拍照
            std::this_thread::sleep_for(std::chrono::milliseconds(2500)); // 等待灯点亮稳定
            start_take_a_picture(1); //标定拍 L0

            // L1 关灯标定拍照
            set_brighten(0); // 关灯
            std::this_thread::sleep_for(std::chrono::milliseconds(2500)); // 等待灯熄灭稳定
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(2500)); // 等待灯点亮稳定 + 补偿画面延迟
            ...
            while (!photo_captured.load() && wait_ms < max_wait_ms) {
            ...
            if (!photo_captured.load()) {
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(max_wait_ms - wait_ms));
            }

            // 第二阶段：关灯拍照（L1 = 无灯照）
            set_brighten(0); // 关灯
            std::this_thread::sleep_for(std::chrono::milliseconds(2500)); // 等待灯熄灭稳定 + 补偿画面延迟
            ...
            // 注意：on_leaving_point() 延后到 run_post_patrol_diff 执行完毕后调用
            // 以便对比检出损伤时也能标记 damage_seen 并落盘
        }
    ...
    if (first_pos != preset_positions.end()) {
...
END:
    ...
    // ★ 所有点位巡检及对比完成后，统一进入"离开"状态
    // 此时 damage_seen 已包含开关灯检测 + 对比检测的结果，再录 3 秒后落盘
    if (!is_calibrate) {
        on_leaving_point();
    }
...
int Camera::set_ptz(int horizontal, int vertical, int brightness)
{
    ...
    if (rc == -1) {
        ...
        // 尝试重新连接并重试一次
        //if (connect_modbus()) {
        //    rc = modbus_write_registers(modbus_ctx, MODBUSPTZ, 2, regs);
        //    if (rc == -1) {
        //        WTALOGI("Retry failed to write PTZ registers: %s", modbus_strerror(errno));
        //        return -1;
        //    }
        //} else {
        //    return -1;
...
int Camera::set_brighten(int brightness)
{
    ...
    // 同时写入两种灯的保持寄存器（白光灯和红外灯 但大云台只会装红外灯 小云台只会装白光灯）
    int rc = modbus_write_registers(modbus_ctx, 0x44A5, 2, brightness_reg); // 即刻亮灭的白光灯

    int rc2 = modbus_write_register(modbus_ctx, 0x4469,brightness_reg[0]); // 光敏控制开关的红外灯
